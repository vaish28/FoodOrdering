...
class Provider extends Details 
{
	...
	{
		teleno=tele;
		String str="pno,'"+name+"','"+service+"',"+teleno+",'"+address+"','"+category+"',"+tcost+","+moncost;
		return str;
	}
	void setprovno(int pn)
	{
		id=pn;
	}
	...
	void upmen(Scanner sc,Statement stmt)
	{
		mm.updatespecific(sc,stmt,id);
	}
	
	void upmenentire(Scanner sc,Statement stmt)
	{
		mm.updateEntire(sc, stmt, id);
		...
		try
		{
		stmt.executeUpdate("update provdetails set scost = "+totalcost+ ", mcost = "+monthlycost+" where pno ="+id);
